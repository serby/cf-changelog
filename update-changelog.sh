...
(git log $last..HEAD --no-merges --format='* %s' | egrep -v '^\* (>|\d)' | sort -u | sed -e 's/\[PT #\(.*\)]/[PT \1](https:\/\/www.pivotaltracker.com\/story\/show\/\1)/' | sed -e 's/\[FD #\(.*\)]/[FD \1](https:\/\/clocklimited.freshdesk.com\/helpdesk\/tickets\/view\/\1)/' ; echo '' ; cat $changelog)  >> $changelog.tmp
